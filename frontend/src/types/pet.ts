...
export interface PetProfile {
  ...
  // Alert management — allows users to dismiss, hide, or postpone clinical alerts
  dismissedAlerts?: string[];                    // condition strings permanently dismissed
  hiddenAlerts?: { condition: string; hiddenAtLifeStage: string }[];  // hidden for current life stage
  postponedAlerts?: { condition: string; postponeUntilStage: string }[];  // postponed until a future life stage

  // Milestone management
  milestoneStatus?: Record<string, {
    status: 'completed' | 'acknowledged' | 'dismissed';
