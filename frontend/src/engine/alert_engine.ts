  ...
  if (pet.medicalHistory) {
    ...
    if (history.onNsaids && history.onCorticosteroids) {
      alerts.push({
        ...
        message: 'Very Important: Concurrent use of NSAIDs and Corticosteroids detected. High risk of GI perforation/ulceration.'
      });
    }
    ...
    const isBrachy = allBreedNames.some(name => 
      !name.toLowerCase().includes('galgah') &&
    ...
    if (isBrachy && (history.bcsScore > 6 || (pet.healthLogs.length > 0 && pet.healthLogs[pet.healthLogs.length-1].bcsScore > 6))) {
      alerts.push({
        severity: AlertSeverity.WARNING,
        label: 'BOAS Risk',
        message: 'Maintaining a healthy body condition may help reduce breathing strain in brachycephalic dogs. Ask your veterinarian about body condition, safe activity, and BOAS (brachycephalic obstructive airway syndrome) concerns.'
      });
    }
    ...
    if (isMDR1Breed) {
      alerts.push({
        ...
        message: 'Some dogs in this breed may carry an MDR1 drug-sensitivity variant. Ask your veterinarian whether genetic testing or medication precautions are appropriate; do not change or withhold medication without veterinary guidance.'
      });
    }
