...
// Idempotent DDL mirroring `frontend/db/001_coparent.sql` plus the admin
// support tables. Running this on first use makes the cloud store
// self-provisioning: a fresh Neon database works without a manual migration
// step, and an already-migrated database is left untouched (every statement is
// guarded with IF NOT EXISTS / a duplicate-object catch).
const DDL = [
  ...
  // Admin support tables (owner-only /admin surface). Self-provisioning and
  // idempotent so a fresh or legacy database gains them without a migration.
  // banned_at is a timestamptz (NULL = not banned); the grants ledger records
  // every entitlement the owner awards so it is auditable; admin_messages
  // carries owner -> user messages surfaced on the user's next login.
